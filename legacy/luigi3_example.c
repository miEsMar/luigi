...
const char *themeItems[] = {
    "panel1",         "panel2",         "selected",
    "border",         "text",           "textDisabled",
    "textSelected",   "buttonNormal",   "buttonHovered",
    "buttonPressed",  "buttonDisabled", "textboxNormal",
    "textboxFocused", "codeFocused",    "codeBackground",
    "codeDefault",    "codeComment",    "codeString",
...
int MyButton2Message(UIElement *element, UIMessage message, int di, void *dp) {
  if (message == UI_MSG_CLICKED) {
    ...
    UIMenuAddItem(menu, 0, "Item 1\tCtrl+F5", -1, MyMenuCallback,
                  (void *)"Item 1 clicked!");
    UIMenuAddItem(menu, 0, "Item 2\tF6", -1, MyMenuCallback,
                  (void *)"Item 2 clicked!");
    UIMenuShow(menu);
  }
...
#else
int WinMain(HINSTANCE instance, HINSTANCE previousInstance, LPSTR commandLine,
            int showCommand) {
#endif
  ...
  UISplitPane *uisplit_topbottom =
      UISplitPaneCreate(&window->e, UI_SPLIT_PANE_VERTICAL, 0.75f);

  // Split top pane (horizontally) into left/right panes.
  UISplitPane *uisplit_top_leftright =
      UISplitPaneCreate(&uisplit_topbottom->e, 0, 0.3f);

  {
    // In the top-left pane - create a single panel taking up the whole pane.
    UIPanel *panel = UIPanelCreate(&uisplit_top_leftright->e,
                                   UI_PANEL_COLOR_1 | UI_PANEL_MEDIUM_SPACING);
    // Panels are by default vertical in layout, so items start at top and go
    // down.
    UIButtonCreate(&panel->e, 0, "Hello World", -1)->e.messageUser =
        MyButtonMessage;
    // Create a new horizontal-layout "sub-panel" and put left and right panels
    // inside it.
    UIPanel *subpanel =
        UIPanelCreate(&panel->e, UI_PANEL_COLOR_1 | UI_PANEL_HORIZONTAL);
    // The left side will layout elements horizontally, with custom borders and
    // gap.
    UIPanel *sub_left =
        UIPanelCreate(&subpanel->e, UI_PANEL_COLOR_1 | UI_PANEL_HORIZONTAL);
    ...
    // The right side will lay out elements vertically (the default), with
    // default medium spacing.
    UIPanel *sub_right =
        UIPanelCreate(&subpanel->e, UI_PANEL_COLOR_1 | UI_PANEL_MEDIUM_SPACING);
    ...
    UIButtonCreate(&panel->e, 0, "Goodbye World", -1)->e.messageUser =
        MyButtonMessage;
  ...
  UISplitPane *uisplit_bottom_leftright =
      UISplitPaneCreate(&uisplit_topbottom->e, 0, 0.3f);

  {
    // Bottom-Left pane.
    UIPanel *panel =
        UIPanelCreate(&uisplit_bottom_leftright->e, UI_PANEL_COLOR_2);
    panel->border = UI_RECT_1(5);
    panel->gap = 5;
    UIButtonCreate(&panel->e, 0, "It's a button??", -1)->e.messageUser =
        MyButton2Message;
    label =
        UILabelCreate(&panel->e, UI_ELEMENT_H_FILL, "Hello, I am a label!", -1);
  }

  {
    // Bottom-Right pane.
    UITabPane *tabPane = UITabPaneCreate(&uisplit_bottom_leftright->e, 0,
                                         "Tab 1\tMiddle Tab\tTab 3");
    ...
    UILabelCreate(&UIPanelCreate(&tabPane->e, UI_PANEL_COLOR_1)->e, 0,
                  "you're in tab 2, bucko", -1);
    UILabelCreate(&UIPanelCreate(&tabPane->e, UI_PANEL_COLOR_1)->e, 0,
                  "hiii!!!", -1);
  }

  UIWindowRegisterShortcut(window,
                           (UIShortcut){.code = UI_KEYCODE_LETTER('T'),
                                        .ctrl = true,
                                        .invoke = MyMenuCallback,
                                        .cp = (void *)"Keyboard shortcut!"});

  {
    ...
    UIMDIChild *child1 =
        UIMDIChildCreate(&client->e, UI_MDI_CHILD_CLOSE_BUTTON,
                         UI_RECT_4(10, 600, 10, 400), "My Window", -1);
    UIPanel *panel1 =
        UIPanelCreate(&child1->e, UI_PANEL_COLOR_1 | UI_PANEL_MEDIUM_SPACING);
    UILabelCreate(&panel1->e, 0, "It's a christmas miracle", -1);
    UIMDIChild *child2 =
        UIMDIChildCreate(&client->e, UI_MDI_CHILD_CLOSE_BUTTON,
                         UI_RECT_4(40, 630, 40, 430), "Second Window", -1);
    UIPanel *panel2 =
        UIPanelCreate(&child2->e, UI_PANEL_COLOR_1 | UI_PANEL_MEDIUM_SPACING);
    UILabelCreate(&panel2->e, 0, "the system is down", -1);
    UIMDIChild *child3 =
        UIMDIChildCreate(&client->e, UI_MDI_CHILD_CLOSE_BUTTON,
                         UI_RECT_4(70, 670, 70, 470), "Third Window", -1);
